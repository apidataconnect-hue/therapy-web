...
| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET`    | `/api/session-plan/{appointmentHash}` | ✅ | Get the existing plan for an appointment (or `null` if none). |
...
## Availability (`/api/therapist/availability`)

Manage weekly availability windows and retrieve suggested time slots for scheduling.

### Suggested slots

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET` | `/api/therapist/availability/slots?date=YYYY-MM-DD` | ✅ | Returns suggested slots for the therapist on the given date. |

Response: `{ "slots": ["10:00", "10:30", ...], "date": "2026-04-14" }`

### Availability windows (weekly schedule)

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET`    | `/api/therapist/availability/windows` | ✅ | List all configured availability windows. |
| `POST`   | `/api/therapist/availability/windows` | ✅ | Replace windows for the days included in the payload. |
| `DELETE` | `/api/therapist/availability/windows/{hash}` | ✅ | Remove a single window. |

#### POST body — array of window objects
```json
[
...
| Field | Type | Required | Notes |
|-------|------|:--------:|-------|
| `dayOfWeek` | int (0–6) | ✅ | 0 = Sunday, 1 = Monday … 6 = Saturday. |
| `startTime` | `HH:mm` | ✅ | |
| `endTime` | `HH:mm` | ✅ | Must be after `startTime`. |
| `isActive` | bool | — | Default `true`. Send `false` to remove the window for that slot. |

> Days not present in the payload keep their existing windows. To clear a day, send entries for it with `isActive: false`.

### Session settings

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET`  | `/api/therapist/availability/settings` | ✅ | Get session duration and slot interval. |
| `POST` | `/api/therapist/availability/settings` | ✅ | Create or update settings. |

#### Settings body
| Field | Type | Required | Notes |
|-------|------|:--------:|-------|
| `sessionDuration` | int (minutes) | ✅ | Default session length, e.g. `50`. |
| `slotInterval` | int (minutes) | — | Interval between suggested slots. Must be ≤ `sessionDuration`. Defaults to `sessionDuration`. |

---

## Patient Invitations — Therapist side (`/api/therapist/patients/invitations`)

Invite patients by email. Generates a unique token, sends an email, and creates a `PatientInvitation` record.

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET`    | `/api/therapist/patients/invitations` | ✅ | List invitations (paginated). Filter with `?status=`. |
| `POST`   | `/api/therapist/patients/invitations` | ✅ | Send a new invitation. |
| `POST`   | `/api/therapist/patients/invitations/{id}/resend` | ✅ | Resend / renew expiry (only if `pending` or `expired`). |
| `POST`   | `/api/therapist/patients/invitations/{id}/cancel` | ✅ | Cancel a pending invitation. |
...
| `firstName` | string | ✅ | |
| `lastName` | string | ✅ | |
| `email` | string | ✅ | Must not have a pending invitation or an existing patient record for this therapist. |
| `phone` | string \| null | — | |
| `internalNotes` | string \| null | — | Visible to therapist only. |
...
## Patient Invitations — Public (`/api/public/invitations`)

No JWT required. Uses `deviceId` + `deviceType` headers.

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET`  | `/api/public/invitations/{token}` | ❌ | Fetch invitation details by token. Also returns `accountExists: bool`. |
| `POST` | `/api/public/invitations/{token}/accept` | ❌ | Accept invitation and create (or link) a patient account. |

### Accept — body
| Field | Type | Required | Notes |
|-------|------|:--------:|-------|
| `password` | string | ✅ if new user | Required only when the email has no existing account. |

### Accept — error codes
| HTTP | `code` | Meaning |
|------|--------|---------|
| `422` | `INVITATION_NOT_PENDING` | Invitation already accepted or cancelled. |
| `422` | `INVITATION_EXPIRED` | Invitation has expired. |
| `422` | `PASSWORD_REQUIRED` | New account needed but no password supplied. |

---

## Patient profile — patient side (`/api/me`)

Endpoints for the authenticated **patient** (`ROLE_PATIENT`). Scoped to the requesting user.

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET` | `/api/me/profiles` | ✅ | List all therapy relationships (one per therapist). |
| `GET` | `/api/me/appointments` | ✅ | List all appointments across therapists. Filter with `?profile={patientHash}`. |

### GET `/api/me/profiles` — response shape
```json
{
  "profiles": [
    {
      ...
      "therapist": { "id": "hash", "name": "Ana García", "specialty": "EMDR" },
      "person": { "firstName": "María", "lastName": "López", "email": "...", "phone": null },
...
Audio transcription and clinical summarisation via OpenAI Whisper. All endpoints accept `multipart/form-data`.

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `POST` | `/api/transcription` | ✅ | Transcribe an audio file. Returns `{ "text": "..." }`. |
| `POST` | `/api/transcription/summarize` | ✅ | Transcribe + generate structured clinical summary. Returns `{ transcript, summary }`. |
| `POST` | `/api/transcription/summarize/{appointmentHash}` | ✅ | Transcribe, summarise and persist directly to the appointment's `SessionNote`. Returns `{ transcript, summary, sessionNote }`. |

### Form fields
| Field | Type | Required | Notes |
|-------|------|:--------:|-------|
| `audio` | file | ✅ | Supported: `m4a`, `mp3`, `ogg`, `wav`, `webm`, etc. |
| `lang` | string | — | Language hint for Whisper. Default `es`. |

### `summary` object shape (returned by `/summarize` and `/summarize/{hash}`)
Fields returned from the AI structured response: `observaciones`, `proximos_pasos`, plus other clinical fields extracted from the transcript.

---

## Telegram integration (`/api/telegram`)

Link and manage the Telegram account associated with the authenticated therapist.

| Method | Path | Auth | Description |
|--------|------|:----:|-------------|
| `GET`    | `/api/telegram/status` | ✅ | Returns `{ "linked": true/false }`. |
| `POST`   | `/api/telegram/link-code` | ✅ | Generate a 6-digit one-time code to send to the bot. Returns `{ "code": "123456", "instruction": "..." }`. |
| `DELETE` | `/api/telegram/unlink` | ✅ | Remove the Telegram link from the authenticated user. |
